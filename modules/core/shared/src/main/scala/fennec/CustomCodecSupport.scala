    ...
    override def encode(t: Proof): Vector[Byte] =
      t match
        case Proof.UserPassword(user, password) =>
          Vector(Headers.USER_PASSWORD) ++ Codec[String].encode(user) ++ Codec[String].encode(password)
        case Proof.Token(bytes) => Vector(Headers.TOKEN) ++ CodecDefaults.given_Codec_Int.encode(bytes.length) ++ bytes
    ...
    new Codec[Message[S, E, U]]:
      ...
      override def encode(t: Message[S, E, U]): Vector[Byte] =
        t match
          ...
          case RequestSession(Some(id)) => Vector(Headers.REQUEST_SESSION) ++ CodecDefaults.given_Codec_UUID.encode(id)
          ...
          case EventMessage(sessionCounter, event)      =>
            val codecE = Codec[E]
            Vector(Headers.EVENT_MESSAGE) ++ CodecDefaults.given_Codec_Long.encode(sessionCounter) ++ codecE.encode(
              event,
            )
          ...
          case SharedEvent(origin, event)               =>
            Vector(Headers.SHARED_EVENT) ++ CodecDefaults.given_Codec_UUID.encode(origin) ++ Codec[E].encode(event)
          case AuthenticateSession(id, proof)           =>
            Vector(Headers.AUTHENTICATE_SESSION) ++ CodecDefaults.given_Codec_UUID.encode(id) ++ Codec[Proof].encode(
              proof,
            )
          case SessionAuthenticated(id, proof, user)    =>
            Vector(Headers.SESSION_AUTHENTICATED) ++ CodecDefaults.given_Codec_UUID.encode(id) ++ Codec[Proof].encode(
              proof,
            ) ++ Codec[U].encode(
              user,
            )
          case AuthenticationFailed(id, proof, message) =>
            Vector(Headers.AUTHENTICATION_FAILED) ++ CodecDefaults.given_Codec_UUID.encode(id) ++ Codec[Proof].encode(
              proof,
            ) ++ Codec[String]
              .encode(message)
